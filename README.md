...
Dynamic Xray (DX) plugin was inspired by the X-ray system on Kindles (see explanation on [Amazon X-Ray on Kindle | All you need to know - YouTube](https://youtu.be/mreow-OrGsU?si=c_3NhHKBDa1BFEvI)). DX differs from the Kindle system in that the user can define items dynamically, while on Kindles these items are "baked into" the ebook. The advantage of the former approach is that you can dynamically add and modify items, but the advantage of the Kindle approach is that it isn't error prone. DX is, because it uses matching of the words in ebook texts to determine whether Xray items are present. Which can lead to erroneous hits. But in at about 95% of cases the matches will be correct.
...
* DX uses mostly buttons with only icons, so without explanatory labels. However, if a button contains a point at the right side of the icon, or a downwards pointing arrow at the left side, this means that you can trigger a popup with help information about the function of that button by longpressing it. An arrow means that a button has more than one action available upon longpress, while a point signifies a one action button. These actions will be presented as buttons at the bottom of the help dialog.
...
## About the code
The Dynamic Xray plugin is structured to resemble a MVC structure:
