...
7. It is possible to add a button to the dictionary lookup results dialog, with which to add the looked-up word to the Xray items. This cannot be done via a patch, but requires a manual edit of dictquicklookup.lua. See
   manual-file-edit-examples/dictquicklookup.lua for an example.
8. It is **⚠️highly recommended to let KOReader display its keyboard in compact mode**, so you have enough space
...
![01 Xray markers in page](images/01-tappable-per-item-xray-markers.png)
01 Xray markers in page:
* Lighting icon in text: upon tap opens UI Xray Page Information dialog.
* Tappable markers (stars) for each item found in the page. Upon tapping, the item will be opened in the Item Viewer.
