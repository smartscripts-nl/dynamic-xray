...
local cols_count = #BOOKINFO_COLS_SET
for _ = 1, cols_count do
    table_insert(bookinfo_values_sql, "?")
end
...
function BookInfoManager:prepareStatements()
    if not self.set_stmt then
...
function BookInfoManager:closeDbConnection()
    if self.db_conn then
        ...
        local stmts = { "get_stmt", "set_stmt", "in_progress_stmt" }
        local stmt
        local count = #stmts
        for i = 1, count do
            stmt = stmts[i]
            if self[stmt] then
                self[stmt]:clearbind():reset():close()
                self[stmt] = nil
            end
        end
...
function BookInfoManager:loadSettings(db_conn)
    ...
    local my_db_conn
    --local close_connection = db_conn and false or true
    if db_conn then
        my_db_conn = db_conn
    else
        self:openDbConnection()
    ...
    if res then
        ...
        local count = #keys
        for i = 1, count do
            self.settings[keys[i]] = tonumber(values[i]) or values[i] --* TEXT db field
        end
    end
end
...
function BookInfoManager:getSetting(key)
    ...
    if self.settings[key] == KOR.registry.skip_unset_ebook_metadata_prop then
...
function BookInfoManager:saveSetting(key, value, db_conn, skip_reload)
    ...
    if value == false or value == KOR.registry.skip_unset_ebook_metadata_prop then
...
--* Bookinfo management
function BookInfoManager:getBookInfo(full_path, get_cover)

    local directory, filename = util.splitFilePathName(full_path)
    ...
    local row
    self:openDbConnection()
    -- self:prepareStatements() already done by openDbConnection:
    --self:prepareStatements()
    row = self.get_stmt:bind(directory, filename):step()
    ...
    for num = 1, cols_count do
        ...
        if col == "cover_w" then
            ...
            if bookinfo["has_cover"] then
                self:generateCover(bookinfo, row, num)
            end
            break
        end
    end
...
function BookInfoManager:extractBookInfo(filepath, cover_specs)
    ...
    local document
    local loaded = true
    --* Alex: cbz files don't support extraction of info:
    if not filepath:match("%.cbz") then
        document = DocumentRegistry:openDocument(filepath)
    else
        loaded = false
    end
    local description_for_statistics
    if document then
        local pages
        ...
        if loaded then
            dbrow.pages = pages
            --* these props are not the raw props in the epub, but the props generated by KOReader:
            local props = FileManagerBookInfo.extendProps(document:getProps(), filepath)
            if next(props) then
                --* there's at least one item
                dbrow.has_meta = "Y"
            end
            if has_content(props.title) then
                dbrow.title = props.title
            end
            if has_content(props.authors) then
                dbrow.authors = props.authors:gsub("\n", "; ")
            end
            if has_content(props.series) then
            ...
            if has_content(props.language) then
                dbrow.language = props.language
            end
            if has_content(props.keywords) then
                dbrow.keywords = props.keywords
            end
            if has_content(props.description) then
                description_for_statistics = props.description
            end
            if cover_specs then
...
function BookInfoManager:setBookInfoProperties(filepath, props)
    ...
    local this_prop_query, stmt
    for k, v in pairs(props) do
        this_prop_query = string.format(base_query, k) --* add column name to query
        stmt = self.db_conn:prepare(this_prop_query)
        if v == false then
            --* convert false to nil (NULL)
            v = nil
        end
...
function BookInfoManager:deleteBookInfo(full_path)
    local directory, filename = util.splitFilePathName(full_path)
    self:openDbConnection()
...
function BookInfoManager:removeMissingFiles()
...
function BookInfoManager:collectSubprocesses()
    ...
    if #self.subprocesses_pids > 0 then
        ...
        local pid
        local count = #self.subprocesses_pids
        for i = count, 1, -1 do
...
function BookInfoManager:extractInBackground(files)
    ...
    local filepath, cover_specs
    local task = function()
        --logger.dbg("  BG extraction started")
        local count = #files
        for idx = 1, count do
            filepath = files[idx].filepath
            cover_specs = files[idx].cover_specs
    ...
    table_insert(self.subprocesses_pids, task_pid)
    ...
    if not self.subprocesses_collector then
        --* there's not one already scheduled
        self.subprocesses_collector = true
        UIManager:scheduleIn(
                self.subprocesses_collect_interval,
                function()
    ...
    while #dirs ~= 0 do
        ...
        for _, d in pairs(dirs) do
            ...
            for f in lfs.dir(d) do
                ...
                elseif attributes.mode == "file" and not util.stringStartsWith(f, "._") and DocumentRegistry:hasProvider(full_path) then
                    table_insert(files, full_path)
                end
            end
        end
        dirs = new_dirs
...
Do you want to prune the cache of removed books?]]
    ...
    if prune then
        ...
        while true do
            ...
            completed, summary = Trapper:dismissableRunInSubprocess(function()
                return self:removeMissingFiles()
            end, info)
    ...
    while i <= nb_files do
        ...
        table_insert(info.movable[1][1], TopContainer:new{
            dimen = Geom:new{
...
function BookInfoManager.isCachedCoverInvalid(bookinfo, cover_specs)
    if not bookinfo.cover_w or not bookinfo.cover_h then
        --* no thumbnail yet
        return true
    end
    if not bookinfo.cover_sizetag then
        --* unset cover_sizetag
        return true
    end
    local img_w, img_h = bookinfo.cover_sizetag:match("(%d+)x(%d+)") --* original image
    if not img_w or not img_h then
        --* old or bad cover_sizetag
        return true
    end
    img_w, img_h = tonumber(img_w), tonumber(img_h)
    local max_img_w = cover_specs.max_cover_w
    local max_img_h = cover_specs.max_cover_h
    if img_w > max_img_w or img_h > max_img_h then
        --* original image bigger than placeholder
        local new_cover_w, new_cover_h = BookInfoManager.getCachedCoverSize(img_w, img_h, max_img_w, max_img_h)
        if new_cover_w > bookinfo.cover_w or new_cover_h > bookinfo.cover_h then
            --* bigger thumbnail needed
            return true
        end
    end
end

function BookInfoManager:hasCover(full_path)
    local bookinfo = self:getBookInfo(full_path, false)
    --* first condition is to fix prevention of sleeping when bookinfo for a book hasn't been retrieved yet:
    return bookinfo and bookinfo.has_cover and not bookinfo.ignore_cover
end

function BookInfoManager:generateCover(bookinfo, row, num)
    ...
    local cover_data = zstd.zstd_uncompress_ctx(cover_blob[1], cover_blob[2]) --*, cover_size
