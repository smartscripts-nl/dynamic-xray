--[[--
ImageWidgetInverted shows an image from a file or memory.

Show image from file example:

        UIManager:show(ImageWidgetInverted:new{
...
--- @class ImageWidgetInverted
local ImageWidgetInverted = Widget:extend{
        ...
        if cached then
            ...
            if itype == "svg" then
                ...
                if not self._bb then
                    logger.warn("ImageWidgetInverted: Failed to render SVG image file:", self.file)
                ...
                if not self._bb then
                    logger.warn("ImageWidgetInverted: Failed to render image file:", self.file)
                    self._bb = RenderImage:renderCheckerboard(width, height, Screen.bb:getType())
                    self._is_straight_alpha = false
                end
            ...
            if self.is_icon and not self.alpha then
                ...
                if bbtype == Blitbuffer.TYPE_BB8A or bbtype == Blitbuffer.TYPE_BBRGB32 then
                    ...
                    icon_bb:fill(KOR.colors.background_inverted)
...
function ImageWidgetInverted:_render()
    if self._bb then -- already rendered
        return
    end
    logger.dbg("ImageWidgetInverted: _render'ing", self.file and self.file or "data", self.width, self.height)
    if self.image then
        self:_loadimage()
    ...
    if self.scale_factor == 0 then
        ...
        if self.width and self.height then
            ...
            logger.dbg("ImageWidgetInverted: scale to fit, setting scale_factor to", self.scale_factor)
        else
    ...
    if self.scale_factor == nil then
        ...
        if self.width and self.height and (self.width ~= bb_w or self.height ~= bb_h) then
            logger.dbg("ImageWidgetInverted: stretching")
    ...
    elseif self.scale_factor ~= 1 then
        ...
        logger.dbg("ImageWidgetInverted: scaling by", self.scale_factor)
    ...
    if self.width and self.height then
        ...
        logger.dbg("ImageWidgetInverted: initial offsets", self._offset_x, self._offset_y)
    end
...
function ImageWidgetInverted:getSize()
    self:_render()
...
function ImageWidgetInverted:getScaleFactor()
    -- return computed scale_factor, useful if 0 (scale to fit) was used
    return self.scale_factor
end

function ImageWidgetInverted:getScaleFactorExtrema()
...
function ImageWidgetInverted:getCurrentWidth()
    return self._bb:getWidth()
end

function ImageWidgetInverted:getCurrentHeight()
    return self._bb:getHeight()
end

function ImageWidgetInverted:getCurrentDiagonal()
    return math.sqrt(self._bb:getWidth()^2 + self._bb:getHeight()^2)
end

-- And now, getters for the original, unscaled dimensions.
function ImageWidgetInverted:getOriginalWidth()
    return self._img_w
end

function ImageWidgetInverted:getOriginalHeight()
    return self._img_h
end

function ImageWidgetInverted:getOriginalDiagonal()
    return math.sqrt(self._img_w^2 + self._img_h^2)
end

function ImageWidgetInverted:getPanByCenterRatio(x, y)
...
function ImageWidgetInverted:panBy(x, y)
    ...
    -- ImageWidgetInverted with a different scale_factor, while keeping center point
    return self.center_x_ratio, self.center_y_ratio
end

function ImageWidgetInverted:paintTo(bb, x, y)
...
-- BlitBuffer zombies
function ImageWidgetInverted:free()
    --print("ImageWidgetInverted:free on", self, "for BB?", self._bb, self._bb_disposable)
...
function ImageWidgetInverted:onCloseWidget()
    -- free when UIManager:close() was called
    self:free()
end

return ImageWidgetInverted
