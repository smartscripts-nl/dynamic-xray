...
local HtmlBoxWidget = require("extensions/widgets/htmlboxwidget")
...
function ScrollHtmlWidget:scrollText(direction)
    ...
    elseif direction < 0 then
        if self.htmlbox_widget.page_number <= 1 then
            ...
            if self.dialog and self.dialog.onForcePreviousTab then
                self.dialog:onForcePreviousTab()
            elseif self.dialog and self.dialog.viewPreviousItem then
                self.dialog:viewPreviousItem()
            end
            return
        end
...
function ScrollHtmlWidget:onScrollUpWithShiftSpace()
    return self:onScrollUp()
end
