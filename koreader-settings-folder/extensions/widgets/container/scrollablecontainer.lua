...
local ScrollableContainer = InputContainer:extend{
    ...
    --* this prop e.g. set to true for closing SeriesManager; see ((make SeriesManager FilesBox closeable with gestures)):
    use_closing_gestures = false,
}
...
function ScrollableContainer:init()
    ...
    if Device:hasKeys() then
        self.key_events = {
            ...
            ScrollPageUpWithShiftSpace   = not ignore.key_pg_back and Input.group.ShiftSpace or nil,
...
function ScrollableContainer:initState()
    ...
    self._max_scroll_offset_x = math_max(0, content_size.w - self.dimen.w)
    self._max_scroll_offset_y = math_max(0, content_size.h - self.dimen.h)
    if self._max_scroll_offset_x == 0 and self._max_scroll_offset_y == 0 then
        ...
        if self._max_scroll_offset_y > 0 then
            ...
            self._max_scroll_offset_x = math_max(0, content_size.w - (self.dimen.w - 3*self.scroll_bar_width))
        end
        if self._max_scroll_offset_x > 0 then
            -- Adding a horizontal scrollbar reduces the available height: recompute
            self._max_scroll_offset_y = math_max(0, content_size.h - (self.dimen.h - 3*self.scroll_bar_width))
            if self._max_scroll_offset_y > 0 then
...
function ScrollableContainer:scrollToRatio(ratio_x, ratio_y)
    if ratio_y then
        ...
        self._scroll_offset_y = dy - math_round(self._crop_h/2)
    ...
    if ratio_x then
        ...
        self._scroll_offset_x = dx - math_round(self._crop_w/2)
...
function ScrollableContainer:_getStepScrollRowAtY(y, check_below)
    local row
    local count = #self.step_scroll_grid
    for i = 1, count do
        row = self.step_scroll_grid[i]
...
function ScrollableContainer:_scrollBy(dx, dy, ensure_scroll_steps)
    ...
    if ensure_scroll_steps and self.step_scroll_grid then
        ...
        elseif orig_y >= self._max_scroll_offset_y and dy >=0 then
            ...
            local top_row, top_row_fully_visible, top_row_content_visible =
                                self:_getStepScrollRowAtY(orig_y, true)
            local bottom_row, bottom_row_fully_visible, bottom_row_content_visible =
                                self:_getStepScrollRowAtY(orig_y + self._crop_h - 1, false)
            --* to keep code sniffers silent:
            top_row_fully_visible = bottom_row_fully_visible
            local new_view_bottom_y = new_y + self._crop_h - 1
            local new_top_row, new_top_row_fully_visible, new_top_row_content_visible =
            ...
            if dy >= 0 then -- Scrolling down
                ...
                if top_row and not top_row_content_visible
                    ...
                    new_top_row, new_top_row_fully_visible, new_top_row_content_visible =
                                self:_getStepScrollRowAtY(new_y, true)
                end
...
function ScrollableContainer:onScrollableSwipe(_, ges)
    ...
    --* this prop e.g. set to true for closing SeriesManager; see ((make SeriesManager FilesBox closeable with gestures)):
    if self.use_closing_gestures then
        local is_closing_gesture = KOR.closingswipes:handle(self.show_parent, _, ges)
        if is_closing_gesture then
            return true
        end
    end
...
function ScrollableContainer:onScrollPageUpWithShiftSpace()
    self:onScrollPageUp()
end
