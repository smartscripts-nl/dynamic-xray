--[[--
ImageWidgetActive shows an image from a file or memory.

Show image from file example:

        UIManager:show(ImageWidgetActive:new{
...
--- @class ImageWidgetActive
local ImageWidgetActive = Widget:extend{
        ...
        if cached then
            ...
            if itype == "svg" then
                ...
                if not self._bb then
                    logger.warn("ImageWidgetActive: Failed to render SVG image file:", self.file)
                ...
                if not self._bb then
                    logger.warn("ImageWidgetActive: Failed to render image file:", self.file)
                    self._bb = RenderImage:renderCheckerboard(width, height, Screen.bb:getType())
                    self._is_straight_alpha = false
                end
...
function ImageWidgetActive:_render()
    if self._bb then -- already rendered
        return
    end
    logger.dbg("ImageWidgetActive: _render'ing", self.file and self.file or "data", self.width, self.height)
    if self.image then
        self:_loadimage()
    ...
    if self.scale_factor == 0 then
        ...
        if self.width and self.height then
            ...
            logger.dbg("ImageWidgetActive: scale to fit, setting scale_factor to", self.scale_factor)
        else
    ...
    if self.scale_factor == nil then
        ...
        if self.width and self.height and (self.width ~= bb_w or self.height ~= bb_h) then
            logger.dbg("ImageWidgetActive: stretching")
    ...
    elseif self.scale_factor ~= 1 then
        ...
        logger.dbg("ImageWidgetActive: scaling by", self.scale_factor)
    ...
    if self.width and self.height then
        ...
        logger.dbg("ImageWidgetActive: initial offsets", self._offset_x, self._offset_y)
    end
...
function ImageWidgetActive:getSize()
    self:_render()
...
function ImageWidgetActive:getScaleFactor()
    -- return computed scale_factor, useful if 0 (scale to fit) was used
    return self.scale_factor
end

function ImageWidgetActive:getScaleFactorExtrema()
...
function ImageWidgetActive:getCurrentWidth()
    return self._bb:getWidth()
end

function ImageWidgetActive:getCurrentHeight()
    return self._bb:getHeight()
end

function ImageWidgetActive:getCurrentDiagonal()
    return math.sqrt(self._bb:getWidth()^2 + self._bb:getHeight()^2)
end

-- And now, getters for the original, unscaled dimensions.
function ImageWidgetActive:getOriginalWidth()
    return self._img_w
end

function ImageWidgetActive:getOriginalHeight()
    return self._img_h
end

function ImageWidgetActive:getOriginalDiagonal()
    return math.sqrt(self._img_w^2 + self._img_h^2)
end

function ImageWidgetActive:getPanByCenterRatio(x, y)
...
function ImageWidgetActive:panBy(x, y)
    ...
    -- ImageWidgetActive with a different scale_factor, while keeping center point
    return self.center_x_ratio, self.center_y_ratio
end

function ImageWidgetActive:paintTo(bb, x, y)
...
-- BlitBuffer zombies
function ImageWidgetActive:free()
    --print("ImageWidgetActive:free on", self, "for BB?", self._bb, self._bb_disposable)
...
function ImageWidgetActive:onCloseWidget()
    -- free when UIManager:close() was called
    self:free()
end

return ImageWidgetActive
