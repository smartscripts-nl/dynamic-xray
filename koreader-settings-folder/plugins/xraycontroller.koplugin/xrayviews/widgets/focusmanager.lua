    ...
    if Device:hasDPad() then
        ...
        if not Device:hasFewKeys() then
            ...
            if type(alternative_keymaps) == "table" then
                local count = #event_keys
                for i = 1, count do
                    key_name = event_keys[i][1]
                    alternative_keymap = alternative_keymaps[key_name]
                    if alternative_keymap then
                        handler_defition = util.tableDeepCopy(event_keys[i][2])
                        handler_defition[1] = alternative_keymap -- replace sample key combinations
                        new_event_key = "Alternative" .. key_name
...
function FocusManager:onPhysicalKeyboardDisconnected()
    ...
    if Device:hasKeys() then
        ...
        for k in pairs(prev_key_events) do
