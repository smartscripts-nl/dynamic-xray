...
function XrayInformation:showGlobalDXHelp(parent, initial_tab)
    local screen_dims = Screen:getSize()
...
<li><strong>families with multiple members:</strong><br />If several members of the same family are mentioned in the book or series, but only one of them is ever mentioned by or in combination with their family name, you could choose to only include this family name in the full name of this one person and to not add it for the other family members. That might improve search/filter results.<br />(by the way: if you want to be able to quickly see all members of a family, you could add them to a tag-group with their family name as tag)<br /> </li>
<li><strong>adding a description/definition from the current e-book:</strong><br />If you want to quickly add a description or definition (instead of a name) from the ebook text to the Xray items: select the (longer) text of that description and in the text selection popup choose <em>+ Xray item</em>. The only thing left to do then is to type the name of the description/definition. This procedure will spare you a lot of typing.<br /> </li>
...
<li><strong>adding quotes</strong><br />from a text selection in the book you can - by tapping on the button <em>+ Xray quote</em> - <em>add your favorite or important quotes to an item</em>. By doing this you'll have them always quickly at hand in the Page Navigator or in the Item Viewer.<br />    After you added quotes to a Xray item, they will be listed in the quotes tab of the Item Viewer. There you'll see a floating speech bubble button, to activate the Quotes Manager. In this Manager you can view quotes, edit them or remove them. Tip: you can use this Manager to add your own notes to individual quotes.</li>
    ...
    <ul>
    <li><strong>overview of all groups</strong><br />A tappable overview of all groups can be called with:<ol><li>Shift+T on your (BT) keyboard</li><li>with a gesture which you assigned to <em>Show the Xray tag-group selector</em> (to be found under Dispatcher-section <em>Reader</em>)</li><li>the tag-group icon (two tags) in several DX dialogs</li></ol><br /> </li>
    <li><strong>inspect a group:</strong><br />tap on the name of a group in the overview of all groups (see above). Or inspect the other members of the tag-group in the <em>tag-groups</em>-tab of the Item Viewer.<br /> </li>
    ...
    <li><strong>adding one item from the Item Editor:</strong><br />Enter one or more names of tag-groups in the field <em>Tags:</em>, under the tab <em>Metadata</em>.<br /> </li>
        ...
        <li><strong>the best way to navigate?</strong>
            ...
            <li>Do you want to see which items are in the current page and <em>you want at the same time to explore relations</em>, then the Page Navigator is you best option. There you can explore relations via the buttons in the side bar. The Page Navigator can be called by <em>longpressing the in-page lightning icon</em>.</li>
            <li>Do you want to <em>explore all items in the current series or book</em>, then call the Items List. With a gesture defined by you. If you tap on a iten in the list, you can then explore relations between item via the context buttons in the footer of the Item Viewer.<br /> </li>
            </ol>
        </li>
        ]]
        .. hotkeys_info ..
    ...
    <li><strong>importing from books in another series:</strong><br />Sometimes your book is part of a follow-up series for a previous series. You can then import the items of that previous series by <em>longpressing the bucket icon</em> and then tap on the button <em>external</em>.</li>
</ul>]])
            },
            {
                tab = _("gestures"),
