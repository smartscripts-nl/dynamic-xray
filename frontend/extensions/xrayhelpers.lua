...
function XrayHelpers:generateParagraphInformation(xray_rects, nr)
    ...
    for i = 1, #xray_items do
        ...
        if not injected_names[name] and (not skip_xray_items or not skip_xray_items[name]) then
            ...
            -- #((use xray hit reliability indicators))
            ...
            -- #((xray items dialog add hit reliability explanations))
...
-- these reliability icons will be injected in the dialog with page or paragraphs information in ((XrayHelpers#generateParagraphInformation)) > ((xray items dialog add hit reliability explanations)):
