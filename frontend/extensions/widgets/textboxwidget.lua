...
local TextBoxWidget = InputContainer:extend{
    ...
    -- if not nil, this must be a table with props:
...
function TextBoxWidget:_splitToLines()
    ...
    local image_num = 0
    local targeted_width = self.width - self.padding_right
...
function TextBoxWidget:_renderText(start_row_idx, end_row_idx)
    ...
    if self.use_xtext then
        for i = start_row_idx, end_row_idx do
            ...
            if line.xglyphs then -- non-empty line
                local face, bolder, glyph, color
                for _, xglyph in ipairs(line.xglyphs) do
                    if not xglyph.no_drawing then
                        face = self.face.getFallbackFont(xglyph.font_num) -- callback (not a method)
                        bolder = self._ptf_char_is_bold and self._ptf_char_is_bold[xglyph.text_index] or false
    ...
    local line, pen_x, line_text, ellipsis_width
    for i = start_row_idx, end_row_idx do
        line = self.vertical_string_list[i]
        pen_x = 0 -- when alignment == "left"
        ...
        line_text = self:_getLineText(line)
        if self.line_with_ellipsis and i == self.line_with_ellipsis then
            -- Requested to add an ellipsis on this line
            ellipsis_width = RenderText:getEllipsisWidth(self.face, self.bold)
...
function TextBoxWidget:paintTo(bb, x, y)
    ...
    --! hotfix for crash because of missing blitbuffer:
    if self._bb then
...
function TextBoxWidget:_getXYForCharPos(charpos)
    ...
    if self.use_xtext then
        ...
        if line.xglyphs then -- non-empty line
            ...
            local xglyph
            local count = #line.xglyphs
            for i = 1, count do
                xglyph = line.xglyphs[i]
                ...
                if xglyph.is_cluster_start then -- ignore non-start cluster glyphs
                    if x < xglyph.x1 then
                        if xglyph.cluster_len <= 1 then
                            ...
                            dw = xglyph.w / xglyph.cluster_len
