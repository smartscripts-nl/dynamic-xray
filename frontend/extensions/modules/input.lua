...
local Input = {
    ...
    group = {
        ...
        --* use this group in this format: EventHandlerWithShiftSpace = Input.group.ShiftSpace:
        ShiftSpace = { { "Shift", { " " } } },
        AltSpace = { { "Alt", { " " } } },
        AltT = { { "Alt", { "T" } } },
        TabNext = { "End", "Tab", "T", "Right" },
        TabPrevious = { "Home", "Left" },


        Cursor = { "Up", "Down", "Left", "Right" },
        -- #((Bigme page forward with menu key)):
        --* screen forward with space, screen back with backspace:
...
function Input:clearTimeouts()
    ...
    if self.input.setTimer then
        local item
        local count = #self.timer_callbacks
        for i = 1, count do
            item = self.timer_callbacks[i]
            if item.timerfd then
                self.input.clearTimer(item.timerfd)
            end
        end
...
function Input:handleKeyBoardEv(ev)
    ...
    if self:isEvKeyPress(ev) and keycode == "F11" and not self.device:isAlwaysFullscreen() then
        KOR.uimanager:broadcastEvent(Event:new("ToggleFullscreen"))
    end

    --* quit on Alt + F4
    --* this is also emitted by the close event in SDL
    if self:isEvKeyPress(ev) and self.modifiers["Alt"] and keycode == "F4" then
        KOR.uimanager:broadcastEvent(Event:new("Close")) --* Tell all widgets to close.
        KOR.uimanager:nextTick(function() KOR.uimanager:quit() end) --* Ensure the program closes in case of some lingering dialog.
    end

    --! handle modifier keys; REALLY needed to support hotkeys like Shift+S etc.:
    if self.modifiers[keycode] ~= nil then
        ...
        elseif ev.value == KEY_RELEASE then
            ...
            --KOR.registry:unset("pressed_modifier")
        end
    end
...
function Input:inhibitInputUntil(set_or_seconds)
    KOR.uimanager:unschedule(self._inhibitInputUntil_func)
    ...
    KOR.uimanager:scheduleIn(delay_s, self._inhibitInputUntil_func)
