...
local Font = {
    ...
    fontmap = {
        ...
        smalltfont_bigme = "NotoSans-Bold.ttf",
        x_smalltfont_bigme = "NotoSans-Bold.ttf",
        ...
        --* mono is an alias by me for smallinfont:
...
local _getFallbackFont = function(face_obj, num)
    ...
    if face_obj.fallbacks[num] ~= nil then
        --* (false means: no more fallback font)
    ...
    for _, fontname in pairs(Font.fallbacks) do
        if fontname ~= realname then
            --* Skip base one if among fallbacks
...
function Font:getFace(font, size, faceindex)
    ...
    if not font or type(font) == "table" then
        --local info = Tables:tableToText(font)
        --logger.err("Font niet compleet:", info)
        font = "x_smallinfofont"
        size = 18
    end
    ...
    --! to prevent crash in Profiles menu:
    if not size then
        size = 20
    end
    ...
    if type(realname) == "table" then
        if realname.realname then
            realname = realname.realname
        else
            realname = "NotoSans-Regular.ttf"
        end
    end
    ...
    if face_obj then
        ...
        font, builtin_font_location = self:loadMissingFont(font, realname)
...
function Font:getDefaultDialogFontFace(add_size)
    ...
    if not add_size then
        KOR.registry.default_dialog_font = face
    end
    return face
end
...
function Font:getFontFamily(family, fontsize)
    ...
    for k, v in FFIUtil.orderedPairs(data) do
        if k == family then
            if not fontsize then
                fontsize = data[k]["font_size"]
                self.garbage = v
            end
            fontfamily = data[k]["font_face"]
            break
        end
    end
    if not fontsize then
    ...
    if fontfamily then
        local font_filename, font_faceindex = cre.getFontFaceFilenameAndFaceIndex(fontfamily)
        if font_filename and font_faceindex then
            return self:getFace(font_filename, fontsize, font_faceindex)
        end
    end
    return nil
end

function Font:setFontByTextLength(text, font)
    ...
    --* when overrule font defined:
    if font and type(font) == "text" then
        return self:getFace(font), KOR.registry.line_height
    end
    --* text parameter not always given:
    if not text then
        return self:getFace("x_smallinfofont"), KOR.registry.line_height_red_hat_text
    end

    local fontsize = 17
    local line_endings_count = KOR.strings:substrCount(text, "\n")
    local is_poem = line_endings_count > 3
    local font_breakpoint = 600
    if is_poem then
        font_breakpoint = 400
    end
    if string.len(text) > font_breakpoint or line_endings_count > 25 then
        fontsize = 16
        --* for very long poems:
        if line_endings_count > 25 then
            fontsize = 13
        end
    end

    --* for longer texts, use a smaller font:
    return self:getFace("x_smallinfofont", fontsize), KOR.registry.line_height_red_hat_text
end

--* not really needed anymore, because in Font Profiles we now use Font:getFontFamily instead of Font:getFace:
function Font:loadMissingFont(font, realname)
    local builtin_font_location = FontList.fontdir .. "/" .. realname

    if not lfs.attributes(builtin_font_location) then

        builtin_font_location = DataStorage:getDataDir() .. "/fonts/host/" .. font
    end

    return font, builtin_font_location
end
